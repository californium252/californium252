...
static void SetupCaliforniumUtilArgs(ArgsManager &argsman)
{
...
{
    SetupCaliforniumUtilArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " californium252-util utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  californium252-util [options] [commands]  Do stuff\n";
