...
const char * const CALIFORNIUM_CONF_FILENAME = "californium252.conf";
const char * const CALIFORNIUM_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/californium252/californium252/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //californium252-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", CALIFORNIUM_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "californium252";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Californium
    // macOS: ~/Library/Application Support/Californium
    // Unix-like: ~/.californium252
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Californium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Californium";
#else
    // Unix-like
    return pathRet / ".californium252";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CALIFORNIUM_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Californium Core copyright is not removed by accident
    if (copyright_devs.find("Californium Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Californium Core developers";
    }
