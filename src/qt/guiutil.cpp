...
#include <qt/californium252addressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Californium address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new CaliforniumAddressEntryValidator(parent));
    widget->setCheckValidator(new CaliforniumAddressCheckValidator(parent));
}

bool parseCaliforniumURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no californium252: URI
    if(!uri.isValid() || uri.scheme() != QString("californium252"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CaliforniumUnits::parse(CaliforniumUnits::C252, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCaliforniumURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCaliforniumURI(uriInstance, out);
}

QString formatCaliforniumURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("californium252:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CaliforniumUnits::format(CaliforniumUnits::C252, info.amount, false, CaliforniumUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCaliforniumConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", CALIFORNIUM_CONF_FILENAME));
    ...
    /* Open californium252.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Californium.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Californium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Californium (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Californium*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "californium252.desktop";
    return GetAutostartDir() / strprintf("californium252-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a californium252.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Californium\n";
        else
            optionFile << strprintf("Name=Californium (%s)\n", chain);
