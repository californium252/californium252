    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return CaliforniumUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, CaliforniumUnits::SeparatorStyle::NEVER);
            else
                return CaliforniumUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+CaliforniumUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
