Californium Core version 0.18.0 is now available from:

  <https://californium252core.org/bin/californium252-core-0.18.0/>
...
`/Applications/Californium-Qt` (on Mac) or `californium252d`/`californium252-qt` (on
Linux).
...
recommended to use Californium Core on unsupported systems.
...
10.10. Additionally, Californium Core does not yet change appearance when
...
`contrib/init/californium252d.service`, it has been changed to use
`/var/lib/californium252d` as the data directory instead of
`~californium252/.californium252`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/californium252d` will
exist has enough space (check using `df -h /var/lib/californium252d`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/californium252/californium252/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://californium252core.org/en/contact/) the Californium Core
...
The systemd init file (`contrib/init/californium252d.service`) has been changed
to use `/var/lib/californium252d` as the data directory instead of
`~californium252/.californium252`. This change makes Californium Core more consistent with
...
`datadir` option in `/etc/californium252/californium252.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/californium252/californium252.conf`.


Documentation
-------------

- A new short [document](https://github.com/californium252/californium252/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/californium252/californium252/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/californium252/californium252/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/californium252/californium252/blob/master/doc/californium252-conf.md)
  about the `californium252.conf` file describes how to use it to configure
  Californium Core.

- A new document introduces Californium Core's BIP174 [Partially-Signed
  Californium Transactions
  (PSBT)](https://github.com/californium252/californium252/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/californium252/californium252/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Californium-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Californium Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/californium252/californium252/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/californium252/californium252/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Californium Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Californium Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/californium252/californium252/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/californium252/californium252/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart californium252d with the `-deprecatedrpc=generate`
...
- A new `californium252-wallet` tool is now distributed alongside Californium
...
This section describes planned changes to Californium Core that may affect
other Californium software and services.

- Since version 0.16.0, Californium Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Californium Core 0.20 (expected about
  a year after 0.18), Californium Core will default to native segwit
  ...
  to bech32 addresses, and if the Californium Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Californium Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Californium Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Californium Core
...
- A sub-project of Californium Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Californium Core.  See their [project
  page](https://github.com/californium252-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Californium Core's own implementation, although entropy
  gathered by Californium Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Californium Core a
...
- On macOS, Californium Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `californium252_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libcalifornium252_server.a` (jnewbery)
...
- #13926 [Tools] californium252-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like californium252d (MarcoFalke)
...
- #14521 qt, docs: Fix `californium252-qt -version` output formatting (hebasto)
...
- #11625 Add CaliforniumApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libcalifornium252_server` (laanwj)
...
- #15154 Configure: californium252-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/californium252-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Californium Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libcalifornium252_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in californium252-util-test.py (isghe)
...
- #12775 Integration of property based testing into Californium Core (Christewart)
...
- #12246 Bugfix: Only run californium252-tx tests when californium252-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_californium252` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_californium252` (MarcoFalke)
...
- #14186 californium252-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_californium252.conf` message on startup if conf file exists (leishman)
...
- #12255 Update californium252.service to conform to init.md (dongcarl)
...
- #14497 Add doc/californium252-conf.md (hebasto)
...
- #15354 Add missing `californium252-wallet` tool manpages (MarcoFalke)
...
- #15765 correct californium252consensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support californium252 folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/californium252/).
