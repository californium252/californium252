...
Californium Core version 0.21.0 is now available from:

  <https://californium252core.org/bin/californium252-core-0.21.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Californium
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Californium Core on
unsupported systems.

From Californium Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/californium252/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Californium Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Californium Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/californium252/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/californium252/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/californium252/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/californium252/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/californium252/bips/blob/master/bip-0342.mediawiki)),
...
- A new `californium252-cli -netinfo` command provides a network peer connections
...
- A new `californium252-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `californium252-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `californium252-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Californium Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `californium252-wallet` tool which performs the salvage
...
Californium Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`californium252.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Californium Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `californium252-cli createwallet` or `californium252-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/californium252/californium252/issues).
...
Users who already have a Californium Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Californium Core.
...
The change to using descriptors changes the default derivation paths used by Californium Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in C252 per 1,000
  vbytes (C252/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from C252/kB
  to sat/vB and the help documentation in C252/kB is updated to C252/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from C252/kvB to sat/vB and the
  ...
  difference between C252/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in C252/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Californium Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `californium252.conf`
...
- #19284 Add seed.californium252.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in californium252-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add californium252-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in californium252-cli (laanwj)
...
### Build system
- #18504 Drop californium252-tx and californium252-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in CALIFORNIUM_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for californium252-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Californium Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrecalifornium252)
...
- #18817 Document differences in californium252d and californium252-qt locale handling (practicalswift)
