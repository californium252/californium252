...
Californium Core version 0.20.0 is now available from:

  <https://californium252core.org/bin/californium252-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `CALIFORNIUM_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Californium Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Californium Core version 0.18.0.
...
- Testing or debugging of implementations of the Californium P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Californium Core. Californium Core logs debug messages
...
- OpenSSL is no longer used by Californium Core.  (#17265)

- BIP70 support has been fully removed from Californium Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/californium252/californium252/issues/16599),
  [PR #16702](https://github.com/californium252/californium252/pull/16702), and the
  `californium252d help` for more information.  This option is experimental and
...
- All custom settings configured when Californium Core starts are now
...
- The "Start Californium Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://californium252core.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Californium Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Californium Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/californium252-core/HWI) for
  signing.  Future versions of Californium Core should support a GUI option
...
- Californium Core's automatically-generated source code documentation is
  now available at https://doxygen.californium252core.org.  (#17596)
...
- The `californium252-cli` utility used with the `-getinfo` parameter now
...
- The `californium252-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `californium252strings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_californium252-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBCALIFORNIUM_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_californium252` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBCALIFORNIUM_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for californium252-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate californium252 schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest californium252.conf (sdaftuar)
...
- #18022 Fix appveyor `test_californium252` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (californium252-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone californium252-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Californium Core files (hebasto)
...
- #17119 Fix broken californium252-cli examples (andrewtoth)
...
- #17503 Remove bitness from californium252-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.californium252core.org (laanwj)
...
- #17648 Rename wallet-tool references to californium252-wallet (hel-o)
...
- #18341 Replace remaining literal C252 with `CURRENCY_UNIT` (domob1812)
...
- #16115 On californium252d startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from californium252-cli -getinfo (malevolent)
