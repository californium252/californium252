Californium Core version 0.11.0 is now available from:

  <https://californium252.org/bin/californium252-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer C252/kB fee to be
...
For example, add the following to `californium252.conf`:
...
related to the blockchain in the californium252 system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Californium Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Californium Core will 
...
For an example script see [californium252-submittx](https://github.com/laanwj/californium252-submittx).
...
- #5334 `cf87536` libcalifornium252consensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new californium252-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only CaliforniumAmountField
- #5753 `0f44672` Add californium252 logo to about screen
...
- #5809 `46bfbe7` Add californium252-cli man page
...
- #5849 `41259ca` contrib/init/californium252d.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Californium-Qt.app to Californium-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for californium252-tx)
