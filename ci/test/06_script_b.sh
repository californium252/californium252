...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  DOCKER_EXEC ${TEST_RUNNER_ENV} DIR_UNIT_TEST_DATA=${DIR_UNIT_TEST_DATA} LD_LIBRARY_PATH=$DEPENDS_DIR/$HOST/lib "${BASE_BUILD_DIR}/californium252-*/src/test/test_californium252*" --catch_system_errors=no -l test_suite
fi
