...
export P_CI_DIR="${BASE_BUILD_DIR}/californium252-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/californium252-config.h > src/config/californium252-config.h.tmp && mv src/config/californium252-config.h.tmp src/config/californium252-config.h'
fi
